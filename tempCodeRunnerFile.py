...
# Emission factors
EMISSION_FACTORS = {"phone": 0.3, "laptop": 0.6, "tv": 0.4}

# Mock dataset for training ML model
df = pd.DataFrame(
    {
        "phone": [100, 200, 300, 400, 500],
        "laptop": [100, 200, 300, 400, 500],
        "tv": [50, 100, 150, 200, 250],
    }
)
df["emission"] = (
    df["phone"] * EMISSION_FACTORS["phone"]
    + df["laptop"] * EMISSION_FACTORS["laptop"]
    + df["tv"] * EMISSION_FACTORS["tv"]
)

# Train ML model
model = RandomForestRegressor()
model.fit(df[["phone", "laptop", "tv"]], df["emission"])


def predict_emission(phone, laptop, tv):
    features = pd.DataFrame([[phone, laptop, tv]], columns=["phone", "laptop", "tv"])
...
def generate_html_report(
    ...
    if include_badges and actual_emission:
        ...
        if reduction_percent >= 20:
            badge_html = "<div class='badge'>🏅 Eco Hero! You reduced emissions by {:.1f}%!</div>".format(
                reduction_percent
            )
            tips_html = """
        ...
        if is_comparison:
            ...
            <div style="margin-top:20px;">
                ...
                <p>📉 <b>Emission Reduced:</b> {actual_emission - emission} g/day ({reduction_percent:.1f}%)</p>
            </div>
            """
...
def main():
    ...
    actual_emission = predict_emission(phone, laptop, tv)
    ...
    hypo_emission = predict_emission(h_phone, h_laptop, h_tv)
